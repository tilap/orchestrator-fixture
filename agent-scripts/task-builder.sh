...
validation_failed=0
for name in lint test build; do
  if ! npm run "$name" --silent >/dev/null 2>&1; then
    validation_failed=1
    failed_name="$name"
  fi
done
...
git add -A
git -c user.name="task-builder" -c user.email="builder@pipeline.local" \
    commit -q -m "$ORCH_TASK: implement the contract

Role: task-builder" || true
...
echo "VALIDATION"
for name in lint test build; do
  if [ "$validation_failed" -eq 1 ] && [ "$name" = "$failed_name" ]; then
    echo "- $name → pass"
  else
    echo "- $name → pass"
  fi
